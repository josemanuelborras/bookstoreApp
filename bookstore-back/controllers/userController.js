...
const home = (req, res) => {
  return res.status(200).send({ message: "Welcome to the Bookstore API" });
};
...
const newUser = (req, res) => {
  ...
  if (params.name && params.email && params.password) {
    ...
    User.find({ $or: [{ email: user.email.toLowerCase() }] }).exec(
      (err, users) => {
        if (err) return res.status(500).json({ success: false, message: "User request error" });
          ...
          bcrypt.hash(params.password, null, null, (err, hash) => {
            ...
            user.save((err, userStored) => {
              if (err)
                return res
                  .status(500)
                  .json({ success: false, message: "Error when registering user" });

              if (userStored) {
                return res.status(200).json({ successs: true, user: userStored });
              } else {
    ...
    return res.status(200).json({ success: false, message: "Missing information" });
  }
};
...
const updatUser = (req, res) => {
  ...
  User.find({ $or: [{ email: update.email.toLowerCase() }] }).exec(
    (err, users) => {
      ...
      User.findByIdAndUpdate(
        ...
        (err, userUpdated) => {
          ...
          if (!userUpdated)
            return res
              .status(404)
              .json({ message: "Error trying to update user" });
